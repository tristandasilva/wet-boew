...
const componentName = "wb-paginate",
	...
	init = function( event ) {
		...
		if ( elm ) {
			...
			paginationElm.setAttribute( "role", "navigation" );
			paginationElm.setAttribute( "aria-label", "pagination" );
	...
	generateUI = function( elm ) {
		...
		if ( pagesCount > 1 ) {
			...
			prevLI += "<button type=\"button\" class=\"paginate-prev\" aria-controls=\"" + elm.id + "\" aria-label=\"" + i18nText.prv + "\">" + i18nText.prv + "</button>";
			prevLI += "</li>";
			...
			nextLI += "<button type=\"button\" class=\"paginate-next\" aria-controls=\"" + elm.id + "\" aria-label=\"" + i18nText.nxt + "\">" + i18nText.nxt + "</button>";
			nextLI += "</li>";
			paginationUI += nextLI;
			paginationUI += "</ol>";
		}
